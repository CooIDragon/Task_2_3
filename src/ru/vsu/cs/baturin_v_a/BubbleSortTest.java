...
import ru.vsu.cs.baturin_v_a.util.LinkedBlockedQueueUtils;
import ru.vsu.cs.baturin_v_a.util.LinkedListQueueUtils;
...
public class BubbleSortTest {
    ...
    public void testNormalLinkedListQueue() throws Exception {
        ...
        LinkedListQueue<Integer> queue = LinkedListQueueUtils.intArrayToList(queueArr);

        bubbleSort.sort(queue);

        int[] queueResultArr = LinkedListQueueUtils.intListToArray(queue);
    ...
    public void testNormalStandardQueue() {
        BubbleSort bubbleSort = new BubbleSort();
        int[] queueArr = {3, 8, 4, 2, 0};
        LinkedBlockingQueue<Integer> queue = LinkedBlockedQueueUtils.intArrayToList(queueArr);

        bubbleSort.sort(queue);

        int[] queueResultArr = LinkedBlockedQueueUtils.intListToArray(queue);
        int[] expectedResult = {0, 2, 3, 4, 8};
